...
Class DB {
	...
	private $db_config;
	/*
    private $db_config["hostname"] = "localhost"; //服务器地址
    private $db_config["username"] = "xsuda"; //数据库用户名
    private $db_config["password"] = "xiansuda"; //数据库密码
    private $db_config["database"] = "xiansuda"; //数据库名称
    private $db_config["charset"] = "utf8";//数据库编码
    private $db_config["pconnect"] = 1;//开启持久连接
    private $db_config["log"] = 1;//开启日志
    private $db_config["logfilepath"] = './';//开启日志	
	*/
    ...
    public function __construct($db_config) {
        ...
        $this->is_log = $this->db_config["log"];
        if($this->is_log){
            $handle = fopen($this->db_config["logfilepath"]."dblog.txt", "a+");
            $this->handle=$handle;
        }
    }
