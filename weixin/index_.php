...
*/ 
   ...
   if(!isset($_REQUEST['order_id'])){
	...
	$db_config["username"] = "xiansuda"; //数据库用户名
	$db_config["password"] = "xiansuda"; //数据库密码
	$db_config["database"] = "xsuda"; //数据库名称
	...
	function getOrderTotal($order_id){
		global $db;
		$sql="select order_id from c_order_info where order_sn = ".$order_id." and user_id = ".$_SESSION['user_id'];
		$result=$db->get_one($sql);
		if(empty($result)){
			  echo "<script>alert('该订单无效！');window.location.href='../'</script>";
			  exit;
		}
		$sql="select * from c_order_goods where order_id = ".$result['order_id'];
		$data=$db->get_all($sql);
		$total=0;
		foreach($data as $da){
			$total+=$da['goods_number']*$da['goods_attr_price'];
		}
			...
			WeixinJSBridge.invoke(
				...
				function(res){
					if(res.err_msg=='get_brand_wcpay_request:cancel'){
						window.location.href="../index.php?g=home&m=user&a=order"
					}else{
						window.location.href="../index.php?g=home&m=user&a=order&pay_status=2"
					}
				}
			);
		}
